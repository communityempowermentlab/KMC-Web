...
class CronjobModelArvind extends CI_Model {
  ...
  public function getNurseAttendanceList($loungeId){
    ...
    $query = $this->db->query("SELECT l.loungeName,n.*,s.name as nurseName FROM `loungeMaster` l LEFT JOIN nurseDutyChange n on n.loungeId=l.loungeId JOIN staffMaster s on s.staffId=n.nurseId WHERE ((n.addDate BETWEEN '2021-03-01 07:30:00' AND '2021-03-02 07:30:00') OR (n.modifyDate BETWEEN '2021-03-01 08:31:00' AND '2021-03-02 08:30:00')) AND l.loungeId IN (137,143,25,34,171,30,19,67,120,119,111,99,73,94,95,148,88,38,97,162,22,24,112,149,150,53,172,55,64) ORDER BY l.loungeName,n.id ASC")->result_array();
    return $query;
  }
