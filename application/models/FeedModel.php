...
class FeedModel extends CI_Model {
    ...
    public function countLike($table,$loungeId,$timelineId) {
        return $this->db->query("select * from ".$table." where type='1' and timelineId='".$timelineId."'")->num_rows();
    }
    ...
    public function GetBabyWeight2($BabyID,$order_by,$WeightDate='',$admisionID='')
    {
        ...
        return $this->db->get_where('babyDailyWeight',array('weightDate'=>$WeightDate,'babyAdmissionId'=>$admisionID))->row_array();
    }

    public function GetSSTDetail($request, $date,$admisionID='')
    {
        $query = $this->db->query("SELECT *  FROM `babyDailyKMC` WHERE `startDate` = '".$date."' and `babyAdmissionId`='".$admisionID."'")->result_array();
        ...
        return ($this->SumOfDuration($sum)!='')? $this->SumOfDuration($sum):'0';
    }


    function SumOfDuration($times) 
    {
