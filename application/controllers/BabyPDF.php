...
class BabyPDF extends CI_Controller {
  public function __construct()
    ...
    $query = $this->db->query("select ba.*,ba.`status` as StatusDis from baby_admission as ba inner join babyRegistration as br on ba.`BabyID`= br.`BabyID` where ba.`LoungeID`=".$LoungeID." and  ba.`add_date` between ".$startDate." and ".$endDate."")->result_array();
        foreach ($query as $key => $value) {

         // $babyAdmisionLastId = $this->db->get_where('baby_admission',array('id'=>$id))->row_array();
         // $getMotherId = $this->db->get_where('babyRegistration',array('BabyID'=>$value['BabyID']))->row_array();
