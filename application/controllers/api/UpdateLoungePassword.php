...
class UpdateLoungePassword extends CI_Controller {
  public function __construct()
    ...
    public function index()
    {
        ...
        $data['nurseId']          = $requestJson[APP_NAME]['nurseId'];
        $data['loungeId']         = trim($requestJson[APP_NAME]['loungeId']);
        $data['oldPassword']      = $requestJson[APP_NAME]['oldPassword'];
        $data['newPassword']      = trim($requestJson[APP_NAME]['newPassword']);
              ...
              if(!empty($headers['token']) && !empty($headers['package']))
              {
                if(($headers['token'] == $getLoungeData['token']) && ($headers['package'] == strtolower(md5(PACKAGE)) || $headers['package'] == strtoupper(md5(PACKAGE))))
                {
              		if ($resultJson == 1 && $resultJson2 == 1) {
              			$this->LoungeModel->updateLoungePassword($data);
              		}else{
