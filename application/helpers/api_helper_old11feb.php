 ...
 */    
    ...
    function allData2($table,$matchWith,$matchingId)
   {
    ...
    function GetHighestValue($select,$table,$Desc_field)
    {
        $tableRecord = & get_instance();
        $tableRecord->load->database();  
        $tableRecord->db->order_by($Desc_field,'DESC');
        $query =$tableRecord->db->get($table)->row_array();
        return $query[$select]; 
    }
   ...
   function singleRowData($table,$matchWith,$matchingId)
   {
    ...
    function saveProfilesImage2($base64,$PROFILE_DIRECTORY,$FOLDER)
    {
        ...
        { 
            $imageName = time().'.jpg';
            //$path = $_SERVER['DOCUMENT_ROOT'];        
            $path = $PROFILE_DIRECTORY.$FOLDER."/".$imageName;
            // echo $path; exit;
            if(imagejpeg($img, $path)) 
                return $imageName;                       
            else
   ...
   function getaddress($lat,$lng)
    ...
    if($dynamicValue != ''){
            $array[APP_NAME]["sync_time"]= $dynamicValue;
        }else{
    ...
    function CheckSameValue($fieldName1,$fieldName2, $msgCode, $msgType){
    ...
    function isAadhar($fieldName, $msgCode, $msgType){
        if(!ctype_digit($fieldName)){
          generateServerResponse($msgCode, $msgType);
        } 
    }
    function isAadharLength($adhar,$msgCode, $msgType)
    {
        if(strlen($adhar) != 12)
        {
            generateServerResponse($msgCode,$msgType);
    ...
    function Messages($res_msg='',$dynamicValue=''){
        $codes = Array(
                        ...
                        '103' => 'Lounge List', 
                        '104' => 'Mother name cannot be blank',                
                        '105' => 'Mother Admitted cannot be blank',
                        '106' => 'Mother photo cannot be blank ',
                        '107' => 'Please Give reason why mother is not admitted',                   
                        '108' => 'Hospital Registeration Number is blank',
                        '109' => 'Mother MCTS number feild is blank',
                        ...
                        '127' => 'Registeration Date & Time is requrie',
