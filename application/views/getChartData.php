...
<?php
 ...
 if(isset($_REQUEST['loungeWiseValue'])){ 
      ...
      $counting            = $this->load->UserModel->getCount();
      $getLoungeName       = $this->db->get_where('loungeMaster',array('loungeId'=>$_REQUEST['loungeWiseValue']))->row_array();
      ...
      $LatestBaby          = $this->load->FacilityModel->getLatestBabyViaLoungeID($_REQUEST['loungeWiseValue'],8); // 8 is limit  
      $LatestMother        = $this->load->FacilityModel->getLatestMotherViaLoungeID($_REQUEST['loungeWiseValue'],8); //8 is data fetch limit
     ...
     // $TotalKMC = $this->load->FacilityModel->overAllLoungeKMC($_REQUEST['loungeWiseValue']);    
      ...
      $getSSCTime = $this->load->FacilityModel->getCurrentBabyInHour($_REQUEST['loungeWiseValue']);
              ...
              foreach ($getSSCTime as $key => $value) {
                $skinTimePerBaby = round($value['totalseconds']/3600,2);
                if($skinTimePerBaby > 0){
      ...
      // bottom graph code
                        ...
                        for ($i = 1; $i < 6; $i++) 
                        {
                           ...
                           $LastDayTimeStamp = date('Y-m-d 23:59:59');  
          ...
          /*                $lastesMonthKmc  = floor(($lastesMonthKmc['totalseconds']/60)/60);
                        ...
                        $hours1   = floor($lastesMonthKmc['totalseconds'] / 3600);
                        $minutes1 = floor(($lastesMonthKmc['totalseconds'] / 60) % 60);
                        $hours2   = floor($lastesMonthKmc1['totalseconds'] / 3600);
                        $minutes2 = floor(($lastesMonthKmc1['totalseconds'] / 60) % 60);
                        $hours3   = floor($lastesMonthKmc2['totalseconds'] / 3600);
                        $minutes3 = floor(($lastesMonthKmc2['totalseconds'] / 60) % 60);
                        $hours4   = floor($lastesMonthKmc3['totalseconds'] / 3600);
                        $minutes4 = floor(($lastesMonthKmc3['totalseconds'] / 60) % 60);
                        $hours5   = floor($lastesMonthKmc4['totalseconds'] / 3600);
                        $minutes5 = floor(($lastesMonthKmc4['totalseconds'] / 60) % 60);
                        $hours6   = floor($lastesMonthKmc5['totalseconds'] / 3600);
                        $minutes6 = floor(($lastesMonthKmc5['totalseconds'] / 60) % 60);

                          $lastesMonthKmc  = $hours1.'.'.$minutes1;
                          $lastesMonthKmc1  = $hours2.'.'.$minutes2;
                          $lastesMonthKmc2  = $hours3.'.'.$minutes3;
                          $lastesMonthKmc3  = $hours4.'.'.$minutes4;
                          $lastesMonthKmc4  = $hours5.'.'.$minutes5;
                          $lastesMonthKmc5  = $hours6.'.'.$minutes6;
                 ...
                 // for Breast Feeding graph in current date
                          ...
                          $DateStart                = $StartCurrentdateTime;
                          $DateEnd                  = $EndCurrentdateTime;
  ...
  ?>
              ...
              <div class="col-md-3 col-sm-6 col-xs-12">
                  <div class="info-box">
                    ...
                    <span class="info-box-number"><a href="<?php echo base_url('staffM/manageStaff/all'); ?>"><?php echo $counting['staff']; ?></a></span>
                  </div>
                 </div>
              </div>
              ...
              <div class="col-md-3 col-sm-6 col-xs-12">
                  <div class="info-box">
                    ...
                    <span class="info-box-number">
                      <a href="<?php echo base_url('motherM/registeredMother/'.$_REQUEST['loungeWiseValue']); ?>"><?php echo $LoungeWiseMothers; ?></a>
                    </span>
                  </div>
                 </div>
              </div>
              <div class="col-md-3 col-sm-6 col-xs-12">
                  <div class="info-box">
                    ...
                    <span class="info-box-number"><a href="<?php echo base_url('babyM/registeredBaby/4/'.$_REQUEST['loungeWiseValue']); ?>"><?php echo $LoungeWiseBabys; ?></a></span>
                  </div>
                 </div>
              </div>
      ...
      <section class="col-lg-6 connectedSortable">       
        ...
        <?php if(count($LatestBaby) > 0) { ?>
          <div class="box box-danger">
                <div class="box-header with-border">
                  <h3 class="box-title">Latest Registered Babies <?php echo ($getLoungeName['loungeName'] != "") ? '('.$getLoungeName['loungeName'].')' : ''; ?></h3>
                </div>
                <!-- /.box-header -->
                    ...
                    foreach ($LatestBaby as $key => $val) { 
                            $this->db->order_by('id','desc'); 
                             ...
                             $motherData = $this->db->get_where('motherRegistration',array('motherId'=>$val['motherId']))->row_array(); 
                                //$GetDangerSign = $this->load->FacilityModel->GetBabyDanger($val['BabyID']);
                             $GetDangerSign = $this->BabyModel->getMonitoringExistData($getAdmissionId['id']);
                               // print_r($GetDangerSign);exit;
                                $babyIconStatus      = $this->load->DangerSignModel->getBabyIcon($GetDangerSign);
                                if($babyIconStatus == '1'){
                            ...
                            <li style="min-height: 195px !important;">
                              ...
                              <?php  if($val['babyPhoto']==NULL){ ?>
                               <img src="<?php echo base_url();?>assets/images/user.png" alt="User Image" class="img-responsive" style="height:100px;width:100px;<?php if(empty($icon)) { echo 'margin-top:-3%'; }else { echo 'margin-top:-28px'; }  ?>">   
                                <?php } else { ?>
                                <img src="<?php echo base_url();?>assets/images/babyImages/<?php echo $val['babyPhoto']; ?>" alt="User Image" class="img-responsive" style="height:100px;width:100px;<?php if(empty($icon)){ echo 'margin-top:-3%'; }else { echo 'margin-top:-28px'; }  ?>">
                                <?php } ?>

                              <a class="users-list-name" href="<?php echo base_url();?>babyM/viewBaby/<?php echo $getAdmissionId['id'];?>/<?php echo $getAdmissionId['status'];?>"><?php echo 'B/O '.(!empty($motherData['motherName']) ? ucwords($motherData['motherName']) : 'UNKNOWN');?></a>
      ...
      <section class="col-lg-6 connectedSortable">
          <?php
            $hours = floor($TotalKMC['kmcTime'] / 3600);
            $minutes = floor(($TotalKMC['kmcTime'] / 60) % 60);
            $seconds = $TotalKMC['kmcTime'] % 60;
          ?>
           <div class="box box-success">
                  <div class="box-header with-border">
             ...
             <!--      <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
                  ...
                  <ul class="fc-color-picker" id="color-chooser">
                      <li><a class="" href="#" style="font-size:15px;color:#fdc80f"><i class="fa fa-square"></i></a>&nbsp;<span style="font-size:15px;">Total KMC Timing (<?php echo $hours.' Hrs '.$minutes.' Mins'; ?>)</span></li> 
                    </ul>
                 </div>
                 </div>
           </div>
                ...
                <div class="box-body no-padding">
                  <ul class="users-list clearfix">
                    ...
                    foreach ($LatestMother as $key => $value) { 
                      $this->db->order_by('id','desc'); 
                      $getAdmissionId = $this->db->get_where('motherAdmission',array('motherId'=>$value['motherId']))->row_array();
                      $this->db->order_by('id','desc');  
                    ...
                    // mother danger sign
                      ...
                      <li style="min-height: 195px !important;">
                        ...
                        <?php  if($value['motherPicture']==NULL){ ?>
                         <img src="<?php echo base_url();?>assets/images/user.png" alt="User Image" class="img-responsive" style="height:100px;width:100px;<?php if(empty($icon)){ echo 'margin-top:-2%'; }else { echo 'margin-top: -28px'; }  ?>">   
                          <?php } else {?>
                          <img src="<?php echo base_url();?>assets/images/motherImages/<?php echo $value['motherPicture']; ?>" alt="User Image" class="img-responsive" style="height:100px;width:100px;<?php if(empty($icon)){ echo 'margin-top:-2%'; } else{ echo 'margin-top:-28px'; }  ?>">
                          <?php } ?>
                        ...
                        <span class="users-list-date"><?php echo date('d-m-Y g:i A', strtotime($value['addDate'])) ?></span>
                      </li>
                  <?php }?>
      ...
      <!--     graph codes here -->
            ...
            <script src="https://static.anychart.com/js/8.0.1/anychart-pie.min.js"></script>
              ...
              <canvas id="areaChart4" style="height:250px;display:none;"></canvas>
...
<script>
 ...
 /********************************* SSC Baby BAR CHART *************************************/
    ...
    $(function () {
    var areaChartCanvas3 = $("#areaChart4").get(0).getContext("2d");
    ...
    //- BAR CHART -
    //-------------
    var barChartCanvas3 = $("#barChart4").get(0).getContext("2d");
    var barChart3 = new Chart(barChartCanvas3);
    var barChartData3 = areaChartData3;
    barChartData3.datasets[0].fillColor = "#4FBDBB";
    barChartData3.datasets[0].strokeColor = "#4FBDBB";
    barChartData3.datasets[0].pointColor = "#4FBDBB";
